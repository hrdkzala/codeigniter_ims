                ...
                if(!$this->Settings->purchase_code || !$this->Settings->envato_username){
                    ...
                    if(! empty($updates->data->updates)) {
                        foreach ($updates->data->updates as $update) {
                            ...
                            echo anchor('settings/install_update/'.substr($update->filename, 0, -4).'/'.(!empty($update->mversion) ? $update->mversion : 0).'/'.$update->version, '<i class="fa fa-upload"></i> ' . lang('install'), 'class="btn btn-xs btn-primary"').'</h3>';
