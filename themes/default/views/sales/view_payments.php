      ...
      <div class="modal-body">
        ...
        <table class="table table-bordered table-condensed table-hover table-striped payments" style="margin-bottom: 0px;">
            ...
            <tr> 
              ...
              <td><?= $p->note; ?></td> 
              <td style="max-width:25px !imprtant; text-align:center;"><a class="tip btn btn-success btn-xs" title="<?= lang("view_payments"); ?>" href="#" onClick="MyWindow=window.open('<?= site_url('sales/payment_note'); ?>?id=<?= $p->id; ?>', 'MyWindow','toolbar=yes,location=no,directories=no,status=no,menubar=yes,scrollbars=yes,resizable=yes,width=1000,height=600'); return false;"><i class="fa fa-file-text-o"></i></a><a class="tip btn btn-warning btn-xs" title="<?= lang("edit_payment"); ?>" href="<?= site_url('sales/edit_payment?id='.$p->id); ?>" data-toggle="modal" data-target="#simModal2"><i class="fa fa-edit"></i></a><a class="tip btn btn-danger btn-xs" title="<?= lang("delete_payment"); ?>" href="<?= site_url('sales/delete_payment?id='.$p->id); ?>"><i class="fa fa-trash-o"></i></a></td>
        	</tr> 
            <?php } ?>
            </tbody>
        </table>    
