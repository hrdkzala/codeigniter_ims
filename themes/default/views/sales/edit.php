...
$due_date = array(
	'name'        => 'due_date',
	'id'          => 'due_date',
	'value'       => $inv->due_date ? $this->sim->hrsd($inv->due_date) : NULL,
	'class'       => 'form-control',
	);
...
<div class="matter">
	<div class="container">
		<?php $attrib = array('class' => 'form-horizontal'); echo form_open("sales/edit/?id=".$id);?>
		...
		<div class="row">
			<div class="col-md-5">
				...
				<div class="form-group">
					...
					<div class="controls"> <?= form_input($due_date);?> </div>
				</div>
				<?php if($inv->recurring != -1) { ?>
				<div class="form-group">
					<label for="recurring"><?= lang("recurring"); ?></label>
					<div class="controls"> <?php 
					$opt = array(0 => lang('no'), 1 => lang('daily'), 2 => lang('weekly'), 3 => lang('monthly'), 4 => lang('quarterly'), 5 => lang('semiannually'), 6 => lang('annually'), 7 => lang('biennially'));
					echo form_dropdown('recurring', $opt, (isset($_POST['recurring']) ? $_POST['recurring'] : $inv->recurring), 'class="form-control" id="recurring"');?> </div>
				</div>
				<?php } ?>
			</div>
				...
				<div class="form-group">
					...
					<div class="controls">
						...
						$st = array(
							...
							echo form_dropdown('status', $st, $inv->status, 'class="status form-control" data-placeholder="'.lang("select")." ".lang("status").'" id="status"');  ?>
						</div>
					</div>
    ...
    <?php if($inv->status != lang('paid') && $inv->status != lang('cancelled')) { ?>
		...
		<tbody>
			...
			foreach($inv_products as $prod){
				...
				<tr id="line<?= $r; ?>">
					...
					<td><?= form_input('quantity'.$r, $prod->quantity, 'id="quantity0'.$r.'" class="form-control input-sm" style="min-width: 70px; text-align: center;"');?></td>
					<td><?= form_input('product'.$r, $prod->product_name, 'id="product0'.$r.'" class="form-control input-sm" style="min-width: 270px;"'); ?></td>
					<?php if($Settings->default_tax_rate) { ?><td><?= form_dropdown('tax_rate'.$r, $tr, $prod->tax_rate_id, 'id="tax_rate0'.$r.'" class="form-control input-sm" style="min-width: 100px;"');  ?></td><?php } ?>
						<td><?= form_input('unit_price'.$r, $prod->unit_price, 'id="price-0'.$r.'" class="form-control input-sm text-right" style="min-width: 100px;"'); ?></td>
					...
					<tr id="line<?= $rw; ?>">
						...
						<td><?= form_input('quantity'.$rw, '', 'id="quantity0'.$rw.'" class="form-control input-sm" style="min-width: 70px; text-align: center;"');?></td>
						<td><?= form_input('product'.$rw, '', 'id="product0'.$rw.'" class="form-control input-sm" style="min-width: 270px;"'); ?></td>
						<?php if($Settings->default_tax_rate) { ?><td><?= form_dropdown('tax_rate'.$rw, $tr, $Settings->default_tax_rate, 'id="tax_rate0'.$rw.'" class="form-control input-sm" style="min-width: 100px;"');  ?></td><?php } ?>
							<td><?= form_input('unit_price'.$rw, '', 'id="price-0'.$rw.'" class="form-control input-sm text-right" style="min-width: 100px;"'); ?></td>
						</tr>
						<?php } 
					} ?>
				</tbody>
			...
			<div class="form-group"> <?= form_submit('submit', lang("update_sale"), 'class="btn btn-primary btn-large"');?> </div>
			<?= form_close();?>
			<div class="clearfix"></div>
		</div>
		<div class="clearfix"></div>
