...
<head>
<meta charset="utf-8">
<title><?= lang('quote')." ".lang("no")." ".$inv->id; ?></title>
...
<tr>
<td class="word_text" colspan="<?= $Settings->default_tax_rate ? '4' : '2'; ?>"><?php if ($inv->inv_total != 0) { $exp = explode($Settings->decimals_sep, $this->sim->formatDecimal( $inv->inv_total)); echo ($this->mywords->to_words($exp[0]))." ".$Settings->major; if(isset($exp[1]) && $exp[1]!=0) { echo " & ". $this->mywords->to_words($exp[1]) ." ".$Settings->minor; } } ?></td>
