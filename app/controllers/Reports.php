...
*/
	...
	function getsales()
	{
		...
		if($this->input->get('cf')){ $cf = $this->input->get('cf'); } else { $cf = NULL; }
		...
		$this->datatables
		->select("sales.id as id, sales.date as date, company.company, reference_no, CONCAT(users.first_name, ' ', users.last_name) as user, customer_name, total+COALESCE(shipping, 0) as total, paid as amount, (total+COALESCE(shipping, 0))-paid as balance, sales.status as status, sales.customer_id as cid, sale_items.product_name as product_name", FALSE)
		->from('sales')
		...
		if($check) { $this->datatables->where('sales.user', $this->session->userdata('user_id')); }
	...
	function getpayments()
	{
		...
		$this->load->library('datatables');
		$this->datatables
		->select("payment.id as id, payment.date as date, payment.invoice_id, customers.company, CONCAT(users.first_name, ' ', users.last_name) as user, amount, note", FALSE)
		->from('payment')
		...
		if($check) { $this->datatables->where('payment.user', $this->session->userdata('user_id')); }
